...
class TestFluxState(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Initialize mirror status tracker
    ...
    def test_flux_state_cycle(self):
        ...
        for i in range(10):
            ...
            # Step 2: Download the main table from Snowflake
            main_table_df = pl.DataFrame(
                self.session.table(self.table_name).to_pandas()
            )
            logging.info("Main table downloaded from Snowflake.")

            if not self.mirror_initialized:
                # Step 3: Initialize FluxState with the main table on the first cycle
                class PatchedDateTime(datetime):
                    _fixed_date = initial_date
                ...
                with patch("fluxstate.datetime", PatchedDateTime):
                    flux_state = FluxState(
                        table=main_table_df, key_column=self.key_column_name, mode="init"
                    )
                logging.info("Initialized FluxState.")
                self.mirror_initialized = True
            else:
                # Step 4: Download the existing mirror table from Snowflake
                ...
                # Re-initialize FluxState with the downloaded mirror table for comparison
                flux_state = FluxState(
                ...
                # Update the mirror table with new changes
                flux_state.table = main_table_df  # Update the table in FluxState
                self.monkey_patch_update(flux_state, simulated_date)

            # Step 5: Save and upload the updated mirror table to Snowflake
            ...
            for col in final_mirror_df.columns:
                if col == self.key_column_name:
                    ...
                    final_mirror_df[col] = final_mirror_df[col].apply(
                        lambda x: [entry if isinstance(entry, dict) else {"date": "", "value": entry} for entry in x]
                    )
